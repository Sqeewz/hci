...
@export var intro_cutscene_texts: Array[String] = [
	...
	"เสียงประกาศเตือนภัยจากลำโพงเก่าๆ ของโรงเรียนดังขึ้นสลับกับเสียงสัญญาณเตือนไซเรน... เมืองทั้งเมืองกำลังจะถูกปิดตาย",
	"ฉันพยายามตั้งสติ พลิกหน้าหนังสือในมือเพื่อหาเบาะแสสุดท้ายเกี่ยวกับเส้นทางลับของสถานีใต้ดินที่อยู่ใต้อาคารเรียนนี้",
	"—ครืนนนนนนน!—",
	"จู่ๆ พื้นดินก็สั่นสะเทือนอย่างรุนแรง แรงกระแทกทำเอาหน้าต่างกระจกข้างตัวแตกละเอียด เศษปูนจากเพดานร่วงกราวลงมา!",
	"ฉันทำได้แค่ก้มหัวลงแนบกับโต๊ะ เอาหมัดสองข้างอุดหูเพื่อบดบังเสียงถล่มอันน่ากลัว... หัวใจฉันเต้นรัวจนแทบจะทะลุออกมา",
	"ถ้าขืนยังอุดอู้อยู่ตรงนี้... ฉันโดนฝังทั้งเป็นแน่!",
	"พอแรงสั่นสะเทือนระลอกแรกเริ่มซาลง ฉันรีบคว้ากระเป๋าเป้ขึ้นมาสะพายหลังทันที",
	"สายตาฉันเหลือบไปเห็นประตูห้อง '2-3' ที่พังอ้าออก ซากปรักหักพังภายนอกเต็มไปด้วยละอองฝนและกลุ่มควันสลัว",
	"ไม่มีเวลาให้ลังเลอีกแล้ว... ฉันออกแรงก้าวขาแล้ววิ่งสุดชีวิตมุ่งตรงไปที่ประตูนั้น!"
]

@export_group("Boss 1 Cutscene Subtitles (Level 8)")
@export var boss1_in_texts: Array[String] = [
	"ฉันวิ่งมาจนสุดทางเดินชั้น 2... แต่ทว่าเบื้องหน้ากลับถูกขวางด้วยประตูเหล็กหนาที่เป็นทางออกฉุกเฉิน (EMERGENCY EXIT)",
	"หน้าจอระบบดิจิทัลกระพริบไฟสีแดงฉาน พร้อมข้อความตอกย้ำความสิ้นหวัง: ENTER CODE",
	"บ้าจริง! เพราะระบบหลักล้มเหลว ประตูเลยล็อคอัตโนมัติงั้นเหรอ? ฉันต้องรีบนึกรหัสผ่านจากสมุดบันทึกเล่มนั้นให้ได้..."
]
@export var boss1_out_texts: Array[String] = [
	"—ตื๊ด!— เสียงสัญญาณอนุมัติดังขึ้น พร้อมกับกลไกประตูเหล็กที่หนักอึ้งค่อยๆ เหวี่ยงเปิดออก",
	"ภาพตรงหน้าทำให้ฉันแทบหยุดหายใจ... ท้องฟ้าเหนือเมืองถูกปกคลุมด้วยกลุ่มควันดำทะมึน เปลวไฟลุกท่วมอาคารสูงหลายแห่งที่กำลังพังทลายลงมา",
	"สายฝนยังคงกระหน่ำซ้ำเติมความมืดมิด ฉันกระชับเป้แน่นก่อนจะวิ่งฝ่าซากปรักหักพังลงสู่ถนนเบื้องล่างอย่างไร้จุดหมาย",
	"ฉันวิ่งหลบเศษปูนที่ร่วงกราวมาตามท้องถนน จนกระทั่งมาหยุดอยู่หน้าขอบหน้าต่างกระจกของร้านขายเครื่องใช้ไฟฟ้าเก่าๆ",
	"แสงไฟจากหน้าจอทีวีโชว์รูมยังคงทำงานอยู่ มันกำลังถ่ายทอดสดข่าวด่วนฉุกเฉิน ภาพบนจอแสดงให้เห็นฝูงชนจำนวนมหาศาลที่กำลังเดินอพยพอย่างโกลาหล",
	"แถบข้อความข่าวด่วนประกาศกร้าว: 'เนื่องจากภัยพิบัติขนาดใหญ่ ขอให้ประชาชนทุกคนเร่งอพยพออกจากเขตเมือง และมุ่งหน้าเข้าสู่เขตพื้นที่ป่าโดยด่วน!'",
	"อพยพเข้าป่างั้นเหรอ...? เมืองทั้งเมืองกำลังจะล่มสลายแล้วจริงๆ สินะ... ฉันไม่มีทางเลือกอื่นแล้ว นอกจากต้องตามกระแสคนพวกนั้นไป"
]
...
func show_intro_cutscene() -> void:
	var slides = []
	for i in range(intro_cutscene_texts.size()):
		var img_path = "res://assets/cuts/cut1.png"
		var shake = false
		
		if i >= 3 and i <= 6:
			img_path = "res://assets/cuts/cut2.png"
			if i == 3 or i == 4:
				shake = true
		elif i >= 7:
			img_path = "res://assets/cuts/cut3.png"
...
func handle_boss_win() -> void:
	...
	var slides = []
	for i in range(boss1_out_texts.size()):
		var img_path = "res://assets/cuts/cutBoss1out.png" if i < 3 else "res://assets/cuts/cut4.png"
		slides.append({
			"img": img_path,
			"text": boss1_out_texts[i]
		})
